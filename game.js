...
let config = {
	...
	containerColorBG: "#131313",
	contentColorBG: "#525060",
	...
	imagesCoin: ["images/ist.png", "images/pri.png", "images/pi.png", "images/ib.png", "images/pmi.png", "images/bin.png","images/z1.png","images/z2.png"],
...
function initGame () {
	...
	// Переключениеигры на выбор монетки
...
function checkMoving () {
	...
	if( config.movingItems == 0 ) {
		...
		switch( config.gameState ) {
			...
			// После передвижения монет проверка
			...
			case config.gameStates[2]:
				...
				if( !isStreak( player.selectedRow, player.selectedCol ) && !isStreak( player.posY, player.posX ) ) {
					...
					// если собралось, то удаляется

					config.gameState = config.gameStates[3]

					// Отметим все удаляемые монеты
				...
				break;
			// После удаления заполняется пустота
...
// Отмечаем монеты для удаления и убираем их из сетки
...
function placeNewGems() {
	...
	// Поиск мест, в которых необходимо создать монету
	...
	// проверяем необходимость их сдвинуть вниз.
	if( gemsPlaced ) {
		config.gameState = config.gameStates[ 3 ];
		checkFalling();
	} else {
		...
		// удаляем те что собрались
		if( combo > 0 ) {
			config.gameState = config.gameStates[ 3 ];
			gemFade();
		} else { 
			// переход в основное состояние игры
			config.gameState = config.gameStates[ 0 ];
			player.selectedRow= -1;
		}
	}
